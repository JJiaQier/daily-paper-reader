...
tldr: 强化学习在物理系统中面临安全约束挑战，探索阶段违反机械限制可能造成不可逆损害。提出通用框架，将深度强化学习的高性能与模型预测控制的形式化安全保证结合，利用MPC离线计算可行状态-动作空间，通过安全滤波器将RL实时动作投影到全局安全集。在非线性1自由度实验台上验证，成功实现安全探索和稳定策略收敛。
source: arxiv
...
motivation: 在主动学习阶段确保严格硬安全约束，避免物理系统因探索造成不可逆损伤。
method: 结合DRL与MPC，利用MPC离线定义安全状态-动作空间，通过安全滤波器投影RL动作到可行集。
result: 在非线性1-DoF实验台上实现安全探索与稳定策略收敛，验证框架有效性。
conclusion: 提出融合MPC安全保证与DRL自适应性能的通用框架，适用于复杂CPS与机器人系统。
---

## 摘要
强化学习（RL）能够直接从数据中合成控制策略，这使得它在复杂信息物理系统（CPS）和机器人技术中极具吸引力。然而，一个持续存在的挑战是在主动学习阶段确保严格、硬性的安全约束。在现实物理系统中，违反机械限制可能导致不可逆的损坏，因此需要探索严格保持在安全操作区域内。我们提出了一个通用框架，将深度强化学习（DRL）的自适应、高性能特性与模型预测控制（MPC）的形式化安全保证相结合。利用系统动力学的数学模型，离线MPC计算定义了一个可行的状态-动作空间，表示所有保证约束满足的系统状态和控制输入的安全组合。在训练和部署期间，RL智能体的即时动作通过安全滤波器投影到这个全局验证的可行集上。我们在一个非线性单自由度实验室测试平台上系统地评估了我们的通用方法，展示了在物理硬件上成功的探索和稳定的策略收敛。
